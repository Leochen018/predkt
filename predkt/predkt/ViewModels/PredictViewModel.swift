...
final class PredictViewModel: ObservableObject {
    ...
    init() {
        NetworkMonitor.shared.$isConnected
            ...
            .sink { [weak self] connected in
                ...
                if connected {
                            // Clear any leftover offline error when connection returns
                            if self?.errorMessage?.contains("No connection") == true {
                                self?.errorMessage = nil
                            }
                            if !(self?.matchesLoaded ?? false) {
                                Task { await self?.loadMatches() }
                            }
                        }
            }
    ...
    func loadMatches() async {
        ...
        guard NetworkMonitor.shared.isConnected else {
            errorMessage = matches.isEmpty ? "No connection — check your signal" : nil
            return
        }
        ...
        for attempt in 1...3 {
            do {
                ...
                print("❌ loadMatches attempt \(attempt)/3: \(error.localizedDescription)")
                if attempt < 3 {
                    let delay = UInt64(pow(2.0, Double(attempt - 1))) * 1_000_000_000
                    try? await Task.sleep(nanoseconds: delay)
                } else {
                    errorMessage = matches.isEmpty
                        ? "Couldn't load matches — check your connection"
                        : "Couldn't refresh — showing cached matches"
                }
            }
        }
    ...
    func refreshMatches() async {
        guard NetworkMonitor.shared.isConnected else {
            errorMessage = "No connection — can't refresh right now"
            return
        }
        ...
        do {
            let fresh = try await APIManager.forceRefresh()
            ...
            errorMessage = "Couldn't refresh — showing cached matches"
        }
    ...
    func getQuestions(for match: Match) -> [Question] {
        ...
        if !r.isEmpty { q.append(Question(category: "FULL-TIME RESULT", prompt: "Who wins at full time?", icon: "trophy", answers: r)) }
        ...
        if !dc.isEmpty { q.append(Question(category: "DOUBLE CHANCE", prompt: "Cover two of the three outcomes", icon: "shield.lefthalf.filled", answers: dc)) }

        let dnb = ans([
            a(match.home, short: "HOME", o?.dnbHome, "dnb"),
            a(match.away, short: "AWAY", o?.dnbAway, "dnb"),
        ])
        if !dnb.isEmpty { q.append(Question(category: "DRAW NO BET", prompt: "Win or get XP back if it's a draw", icon: "arrow.uturn.left.circle", answers: dnb)) }
        ...
        if !ah.isEmpty { q.append(Question(category: "ASIAN HANDICAP", prompt: "Adjusted result — no draw possible", icon: "scalemass", answers: ah)) }
        ...
        if !wtn.isEmpty { q.append(Question(category: "WIN TO NIL", prompt: "A team wins without conceding", icon: "lock.shield", answers: wtn)) }
        ...
        if !goals.isEmpty { q.append(Question(category: "TOTAL GOALS — OVER/UNDER", prompt: "Choose a goal line for the full match", icon: "soccerball", answers: goals)) }
        ...
        if !exact.isEmpty { q.append(Question(category: "EXACT NUMBER OF GOALS", prompt: "How many goals will be scored in total?", icon: "number.circle", answers: exact)) }

        let oe = ans([
            a("Odd total goals",  short: "ODD",  o?.goalsOdd,  "goals_oe"),
            a("Even total goals", short: "EVEN", o?.goalsEven, "goals_oe"),
        ])
        if !oe.isEmpty { q.append(Question(category: "GOALS — ODD OR EVEN", prompt: "Will total goals be odd or even?", icon: "dice", answers: oe)) }
        ...
        if !htft.isEmpty { q.append(Question(category: "HALF-TIME / FULL-TIME", prompt: "Pick the result at the break AND at full time", icon: "arrow.up.right.circle", answers: htft)) }
        ...
        if !ht.isEmpty { q.append(Question(category: "FIRST HALF — RESULT", prompt: "Who's ahead at the break?", icon: "clock", answers: ht)) }
        ...
        if !sh.isEmpty { q.append(Question(category: "SECOND HALF — RESULT", prompt: "Who wins just the second half?", icon: "clock.arrow.2.circlepath", answers: sh)) }
        ...
        if !htg.isEmpty { q.append(Question(category: "FIRST HALF — GOALS", prompt: "Goals before the half-time whistle", icon: "1.circle.fill", answers: htg)) }
        ...
        if !btts.isEmpty { q.append(Question(category: "BOTH TEAMS TO SCORE", prompt: "Do both sides get on the scoresheet?", icon: "arrow.left.and.right.circle", answers: btts)) }
        ...
        if !bttsW.isEmpty { q.append(Question(category: "BTTS & WINNER", prompt: "Both score — but who takes the points?", icon: "plus.circle", answers: bttsW)) }
        ...
        if bttsH.count >= 2 { q.append(Question(category: "BTTS — FIRST HALF", prompt: "Do both teams score before half time?", icon: "1.circle", answers: bttsH)) }
        ...
        if bttsS.count >= 2 { q.append(Question(category: "BTTS — SECOND HALF", prompt: "Do both teams score after half time?", icon: "2.circle", answers: bttsS)) }

        let bttsBH = ans([
            a("Yes — both score in each half", short: "YES", o?.bttsBothHalves,          "btts_bh"),
            a("No",                            short: "NO",  implied(o?.bttsBothHalves), "btts_bh"),
        ])
        if bttsBH.count >= 2 { q.append(Question(category: "BTTS — BOTH HALVES", prompt: "Both teams score in each half?", icon: "arrow.left.and.right", answers: bttsBH)) }
        ...
        if !sbh.isEmpty { q.append(Question(category: "SCORE IN BOTH HALVES", prompt: "Does one team score in each half?", icon: "arrow.triangle.2.circlepath", answers: sbh)) }
        ...
        if !cs.isEmpty { q.append(Question(category: "CLEAN SHEET", prompt: "Which keeper keeps a shutout?", icon: "hand.raised.slash", answers: cs)) }
        ...
        if !lts.isEmpty { q.append(Question(category: "LAST TEAM TO SCORE", prompt: "Who gets the final goal?", icon: "flag.fill", answers: lts)) }
        ...
        if !ag2.isEmpty { q.append(Question(category: "\(match.away.uppercased()) — GOALS", prompt: "How many does \(match.away) score?", icon: "airplane", answers: ag2)) }
        ...
        if !corners.isEmpty { q.append(Question(category: "CORNERS — TOTAL", prompt: "How many corners in the full match?", icon: "flag", answers: corners)) }
        ...
        if !htc.isEmpty { q.append(Question(category: "CORNERS — FIRST HALF", prompt: "How many corners before the break?", icon: "flag.fill", answers: htc)) }
        ...
        if !cards.isEmpty { q.append(Question(category: "BOOKINGS — TOTAL CARDS", prompt: "How many yellow cards will be shown?", icon: "rectangle.portrait", answers: cards)) }
        ...
        if !shots.isEmpty { q.append(Question(category: "SHOTS — TOTAL", prompt: "How many shots are taken in the match?", icon: "scope", answers: shots)) }
        ...
        if !assist.isEmpty { q.append(Question(category: "PLAYER TO ASSIST", prompt: "Who creates a goal with an assist?", icon: "hand.point.right.fill", answers: assist)) }

        let carded = players(o?.playerToBeCarded, grp: "card", suffix: "booked")
        if !carded.isEmpty { q.append(Question(category: "PLAYER TO BE BOOKED", prompt: "Which player receives a yellow card?", icon: "rectangle.portrait.fill", answers: carded)) }

        let sot = players(o?.playerShotsOnTarget, grp: "sot")
        if !sot.isEmpty { q.append(Question(category: "PLAYER SHOTS ON TARGET", prompt: "Who tests the keeper most?", icon: "scope", answers: sot)) }

        let fouled = players(o?.playerToBeFouled, grp: "fouled")
        if !fouled.isEmpty { q.append(Question(category: "PLAYER TO BE FOULED MOST", prompt: "Who wins the most free kicks?", icon: "figure.fall", answers: fouled)) }
    ...
    func submitPlays(match: Match, myPicksCount: Int) async -> Bool {
        guard NetworkMonitor.shared.isConnected else {
            errorMessage = "No connection — predictions can't be submitted offline"
            return false
        }
