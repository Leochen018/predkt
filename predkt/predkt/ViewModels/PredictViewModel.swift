...
final class PredictViewModel: ObservableObject {
    ...
    static let leaguePriority: [String] = [
        ...
        "Ligue 2", "Primeira Liga", "Eredivisie", "World Cup", "Euro Championship",
    ]
    ...
    struct Answer: Identifiable, Equatable {
        ...
        var communityPercent: Int { min(92, max(8, probability)) } // deterministic
    ...
    // MARK: - Date Helpers
    // ✅ KEY FIX: Compare date strings (YYYY-MM-DD) directly in LOCAL timezone
    // Avoids UTC vs BST mismatch that caused future matches to not appear

    private let localDateFormatter: DateFormatter = {
        let f = DateFormatter()
        f.dateFormat = "yyyy-MM-dd"
        f.timeZone = .current  // Device local timezone
        return f
    }()

    // Convert a raw ISO8601 string → local date string "yyyy-MM-dd"
    private func localDateString(from rawDate: String) -> String {
        let f1 = ISO8601DateFormatter()
        f1.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
        if let d = f1.date(from: rawDate) { return localDateFormatter.string(from: d) }
        let f2 = ISO8601DateFormatter()
        f2.formatOptions = [.withInternetDateTime]
        if let d = f2.date(from: rawDate) { return localDateFormatter.string(from: d) }
        // Last resort: take the first 10 chars of rawDate (already "yyyy-MM-dd")
        return String(rawDate.prefix(10))
    }

    // The selected date as a local "yyyy-MM-dd" string
    private var selectedDateString: String {
        localDateFormatter.string(from: selectedDate)
    }
    ...
    var filteredMatches: [Match] {
        ...
        return matches
            .filter { localDateString(from: $0.rawDate) == target }
            .sorted {
    ...
    // MARK: - Available dates (for showing which days have matches)

    var availableDateStrings: Set<String> {
        Set(matches.map { localDateString(from: $0.rawDate) })
    }

    func hasMatches(on date: Date) -> Bool {
        let target = localDateFormatter.string(from: date)
        return availableDateStrings.contains(target)
    }
    ...
    func loadMatches() async {
        ...
        do {
            ...
            matchesLoaded = true
            print("✅ Loaded \(matches.count) matches across \(availableDateStrings.count) days")
            // Advance to first day with matches if today is empty
            if filteredMatches.isEmpty, let firstDate = nextDayWithMatches(from: Date()) {
    ...
    private func nextDayWithMatches(from date: Date) -> Date? {
        let cal = Calendar.current
        for i in 0..<60 {
            guard let candidate = cal.date(byAdding: .day, value: i, to: date) else { continue }
            if hasMatches(on: candidate) { return candidate }
        }
        return nil
    }
    ...
    func getQuestions(for match: Match) -> [Question] {
        ...
        let exact = ans([a("No goals",short:"0",o?.exactGoals0,"ex0"),a("Exactly 1",short:"1",o?.exactGoals1,"ex1"),a("Exactly 2",short:"2",o?.exactGoals2,"ex2"),a("Exactly 3",short:"3",o?.exactGoals3,"ex3"),a("Exactly 4",short:"4",o?.exactGoals4,"ex4"),a("5 or more",short:"5+",o?.exactGoals5plus,"ex5")])
        ...
        let hg = ans([a("\(match.home) score 1+",short:"1+",o?.homeOver05,"hg05"),a("\(match.home) score 0",short:"0",o?.homeUnder05,"hg05"),a("\(match.home) score 2+",short:"2+",o?.homeOver15,"hg15"),a("\(match.home) score 3+",short:"3+",o?.homeOver25,"hg25")])
        ...
        let ag2 = ans([a("\(match.away) score 1+",short:"1+",o?.awayOver05,"ag05"),a("\(match.away) score 0",short:"0",o?.awayUnder05,"ag05"),a("\(match.away) score 2+",short:"2+",o?.awayOver15,"ag15"),a("\(match.away) score 3+",short:"3+",o?.awayOver25,"ag25")])
        ...
        if let scores = o?.correctScores, !scores.isEmpty {
            q.append(Question(category:"🎯 CORRECT SCORE",prompt:"What's the exact final score?",icon:"number.circle",answers:scores.map { Answer($0.score,short:$0.score,odds:$0.odd,group:"cs_\($0.score)") }))
        }
        if let scores = o?.correctScoresHT, !scores.isEmpty {
            q.append(Question(category:"🎯 CORRECT SCORE — HT",prompt:"Score at half time?",icon:"number.square",answers:scores.map { Answer("HT: \($0.score)",short:$0.score,odds:$0.odd,group:"cs_ht_\($0.score)") }))
        }
        if let scores = o?.correctScoresSH, !scores.isEmpty {
            q.append(Question(category:"🎯 CORRECT SCORE — 2ND HALF",prompt:"Goals in the 2nd half?",icon:"number.square.fill",answers:scores.map { Answer("2H: \($0.score)",short:$0.score,odds:$0.odd,group:"cs_sh_\($0.score)") }))
        }
        ...
        let corners = ans([a("Under 8",short:"U7.5",o?.cornersUnder75,"c75"),a("8+",short:"O7.5",o?.cornersOver75,"c75"),a("Under 9",short:"U8.5",o?.cornersUnder85,"c85"),a("9+",short:"O8.5",o?.cornersOver85,"c85"),a("Under 10",short:"U9.5",o?.cornersUnder95,"c95"),a("10+",short:"O9.5",o?.cornersOver95,"c95"),a("11+",short:"O10.5",o?.cornersOver105,"c105")])
        if !corners.isEmpty { q.append(Question(category:"🚩 TOTAL CORNERS",prompt:"How many corners?",icon:"flag",answers:corners)) }
        let cards = ans([a("Under 2",short:"U1.5",o?.cardsUnder15,"cr15"),a("2+",short:"O1.5",o?.cardsOver15,"cr15"),a("Under 3",short:"U2.5",o?.cardsUnder25,"cr25"),a("3+",short:"O2.5",o?.cardsOver25,"cr25"),a("Under 4",short:"U3.5",o?.cardsUnder35,"cr35"),a("4+",short:"O3.5",o?.cardsOver35,"cr35"),a("5+",short:"O4.5",o?.cardsOver45,"cr45")])
        ...
        let shots = ans([a("Under 9",short:"U8.5",o?.shotsUnder85,"sh85"),a("9+",short:"O8.5",o?.shotsOver85,"sh85"),a("Under 11",short:"U10.5",o?.shotsUnder105,"sh105"),a("11+",short:"O10.5",o?.shotsOver105,"sh105"),a("13+",short:"O12.5",o?.shotsOver125,"sh125")])
        ...
        if !score2.isEmpty { q.append(Question(category:"⚽⚽ SCORE 2+ GOALS",prompt:"Who scores a brace?",icon:"2.circle.fill",answers:score2)) }
        ...
        if !carded.isEmpty { q.append(Question(category:"🟨 PLAYER BOOKED",prompt:"Who picks up a yellow?",icon:"rectangle.portrait.fill",answers:carded)) }
    ...
    private func fallbackQuestions(for match: Match) -> [Question] {[
        ...
        Question(category:"🔀 BOTH TEAMS SCORE?",prompt:"Will both teams score?",icon:"arrow.left.and.right.circle",answers:[
            ...
            Answer("No — one blank",short:"NO",odds:1.60,group:"btts"),
        ]),
    ]}
